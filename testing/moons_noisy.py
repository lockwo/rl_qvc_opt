...
moon_data, moon_labels = ds.make_moons(300, noise=0.15, shuffle=True)
moon_data = MinMaxScaler().fit_transform(moon_data)

#plt.scatter(moon_data[moon_labels == 0][:,0], moon_data[moon_labels == 0][:,1], label='0', color='blue')
#plt.scatter(moon_data[moon_labels == 1][:,0], moon_data[moon_labels == 1][:,1], label='1', color='red')
#plt.show()
...
d = 6
X_train, X_test, y_train, y_test = encode(moon_data, moon_labels, qs)
c = model_circuit(qs, d)
c = c.with_noise(cirq.depolarize(p=0.075))
readout_operators = [cirq.Z(qs[0]), cirq.Z(qs[1])]
...
for it in range(rep):
    ...
    vqc.compile(loss=loss_fn, optimizer=tf.keras.optimizers.Adam(lr=0.01), metrics=['acc'])
...
print("Validation")
print("SAC", np.mean(mlp_mins_val), np.std(mlp_mins_val), "SAC CNN", np.mean(cnn_mins_val), np.std(cnn_mins_val), "Gradient",\
     np.mean(grad_mins_val), np.std(grad_mins_val), "Mixed", np.mean(mixed_mins_val), np.std(mixed_mins_val))
